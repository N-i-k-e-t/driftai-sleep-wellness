  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div><h1 className="text-2xl font-bold">Good Evening</h1><p className="text-drift-muted text-sm">How are you feeling?</p></div>
      ...
      <div className="bg-gradient-to-br from-drift-accent/20 to-purple-900/20 rounded-2xl p-6 border border-purple-900/20 text-center">
        ...
        <p className="text-5xl font-bold text-drift-accent">{sleepScore}</p>
        <p className="text-drift-muted text-xs mt-1">out of 100</p>
      </div>
      ...
      <div className="bg-drift-card rounded-2xl p-4 border border-purple-900/20">
        ...
        <div className="mt-3"><label className="text-xs text-drift-muted">Quality (1-5)</label>
          <div className="flex gap-2 mt-1">{[1,2,3,4,5].map(n => (
            <button key={n} className="flex-1 py-2 rounded-lg bg-drift-bg border border-purple-900/20 text-sm hover:bg-drift-accent/20 transition">{n}</button>
          ))}</div>
        </div>
        <button className="w-full mt-4 bg-drift-accent hover:bg-purple-600 text-white font-medium py-2.5 rounded-xl transition">Save Log</button>
      </div>
    </div>
  )
}
